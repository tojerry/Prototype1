...
#include "V_MM_UnityStandardInput.cginc"
...
#define FRAGMENT_SETUP(x) FragmentCommonData x = \
	FragmentSetup(i.color, i.tex, i.eyeVec, WorldNormal(i.tangentToWorldAndParallax), IN_VIEWDIR4PARALLAX(i), ExtractTangentToWorldPerPixel(i.tangentToWorldAndParallax), IN_WORLDPOS(i));

#define FRAGMENT_SETUP_FWDADD(x) FragmentCommonData x = \
	FragmentSetup(i.color, i.tex, i.eyeVec, WorldNormal(i.tangentToWorldAndLightDir), IN_VIEWDIR4PARALLAX_FWDADD(i), ExtractTangentToWorldPerPixel(i.tangentToWorldAndLightDir), half3(0,0,0));
...
inline FragmentCommonData SpecularSetup (float4 i_tex, fixed4 i_color)
{
	...
	half3 diffColor = EnergyConservationBetweenDiffuseAndSpecular (Albedo(i_tex, i_color), specColor, /*out*/ oneMinusReflectivity);
...
inline FragmentCommonData MetallicSetup (float4 i_tex, fixed4 i_color)
{
	...
	half oneMinusReflectivity;
	half3 specColor;
	half3 diffColor = DiffuseAndSpecularFromMetallic (Albedo(i_tex, i_color), metallic, /*out*/ specColor, /*out*/ oneMinusReflectivity);
...
inline FragmentCommonData FragmentSetup (fixed4 i_color, float4 i_tex, half3 i_eyeVec, half3 i_normalWorld, half3 i_viewDirForParallax, half3x3 i_tanToWorld, half3 i_posWorld)
{
	...
	FragmentCommonData o = UNITY_SETUP_BRDF_INPUT (i_tex, i_color);
